   ...
   public class UploadActions
   {
      ...
      public void ShowWindow(UploadPlatform uploadPlatform = UploadPlatform.Merch)
      {
         ...
         UploadWindowViewModel mainVM = new UploadWindowViewModel(dataFolder)
         {
            ...
            AddTagCmd = new RelayCommand(AddTagCmdInvoke),
      ...
      private void AddTagCmdInvoke(object obj)
      {
         UploadWindowViewModel uploadVM = obj as UploadWindowViewModel;
         if (uploadVM != null)
         {
            if (!string.IsNullOrEmpty(uploadVM.SeedingTag))
            {
               string supportingTag = TagGenerator.GenerateTag(uploadVM.SeedingTag);
               foreach (Shirt shirt in uploadVM.Shirts)
               {
                  if (string.IsNullOrEmpty(shirt.MainTag) || string.IsNullOrEmpty(shirt.SupportingTags))
                  {
                     shirt.MainTag = uploadVM.SeedingTag;
                     shirt.SupportingTags = supportingTag;
                     ShirtData sData = ShirtDTO.MapData(shirt, typeof(ShirtData)) as ShirtData;
                     JsonDataAccess json = new JsonDataAccess();
                     json.SaveShirt(sData);
                  }
               }
               Utils.ShowInfoMessageBox("Done");
            }
            else
            {
               Utils.ShowErrorMessageBox("Main Tag text box is empty");
            }
         }
      }
      ...
      public void ManualLogInCallBack(object obj)
      {
         if (obj is UploadWindowViewModel uploadVM)
         {
            try
            {
               ...
               {
                  upload = new UploadTeePublic(uploadVM.Email, uploadVM.password);
               }
